...
export default function RoomsPage() {
  ...
  const getCount = (key) =>
    key === 'all' ? rooms.length : rooms.filter((r) => r.category === key).length;
      ...
      <section className="flex flex-col lg:flex-row min-h-screen">
          ...
          <div className="lg:pt-8">
            ...
            <nav className="space-y-4">
              {filters.map((f) => (
                ...
                >
                  {f.label}
                  <span className="ml-2 text-xs text-muted">({getCount(f.key)})</span>
                </button>
              ))}
            </nav>
            ...
            <div className="mt-12 pt-8 border-t border-accent">
              ...
              <button className="w-full bg-primary text-background py-3 font-heading text-xs font-semibold tracking-[4px] uppercase hover:bg-surface transition-colors duration-300">
                Contact Concierge
              </button>
            </div>
          </div>
        ...
        <div className="lg:w-[70%] p-8 space-y-[1px] bg-accent">
          {filteredRooms.map((room) => (
            <div
              key={room.id}
              className="bg-background transition-all duration-500 cursor-crosshair h-[400px]"
            >
              <div className="flex flex-col lg:flex-row h-full">
                ...
                <div className="lg:w-1/2 p-8 flex flex-col justify-between">
                  ...
                  <div className="mt-6 flex items-center justify-between">
                    ...
                    <button className="font-body text-xs text-muted tracking-[2px] uppercase hover:text-primary transition-colors">
                      View Details
                    </button>
                  </div>
                </div>
